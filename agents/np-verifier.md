...
- **Rule 1 — Do the whole thing.** Rule 1's mechanical check names *you*: compare the diff against `success_criteria` **plus** the implicit boundary list — empty, boundary, overflow, concurrent, failure. A criterion with no evidence on a boundary case is unproven, and unproven is not Pass. Pressure-tested by `PRS-R01-VERIFIER-PARTIAL`.
