...
function ffMergeSliceWorktree(sliceFullIdStr, targetBranch, cwd, opts) {
  ...
  const beforeSha = _currentHeadSha(cwd);
  ...
  const advanced = !(beforeSha && mergedSha && beforeSha === mergedSha);
  if (!advanced && !allowEmpty) {
    throw new NubosPilotError(
      'worktree-ff-no-op',
      'git merge --ff-only for ' + branch + ' advanced nothing — the slice branch carries no commits '
        + 'the target branch lacks. The task commits most likely landed on the calling branch '
        + '(commit-task without --cwd), so worktree isolation did not happen. If the slice '
        + 'legitimately committed nothing (all tasks gitignored), pass --allow-empty.',
      { slice_full_id: sliceFullIdStr, branch, target_branch: targetBranch || currentBranch, head_sha: mergedSha },
    );
  }
  return { slice_full_id: sliceFullIdStr, branch, target_branch: targetBranch || currentBranch, merged_sha: mergedSha, advanced };
}
