...
test('codex-adapter: capabilities match D-07 spec', () => {
  ...
  // Codex does have an invocable command surface — as skills (`$np-plan-phase`),
  // since `.codex/skills/` is project-scoped while custom prompts are not.
  assert.equal(c.slashCommands, true);
  assert.equal(c.commandSurface, 'skills');
  assert.equal(c.agentsMd, 'AGENTS.md');
  assert.equal(c.textMode, 'auto');
  assert.equal(c.modelResolution, 'profile');
});

test('codex-adapter: paths point at the per-project .codex tree', () => {
  assert.equal(codex.paths.payload, '.codex/nubos-pilot');
  assert.equal(codex.paths.config, '.codex');
  assert.equal(codex.paths.skills, '.codex/skills');
  assert.equal(codex.paths.agents, '.codex/agents');
