...
function milestoneResearchPath(mNum, cwd) {
  return path.join(milestoneDir(mNum, cwd), mId(mNum) + '-RESEARCH.md');
}

function milestoneArchitecturePath(mNum, cwd) {
  return path.join(milestoneDir(mNum, cwd), mId(mNum) + '-ARCHITECTURE.md');
}
...
module.exports = {
  ...
  milestoneResearchPath, milestoneArchitecturePath,
  slicesRoot, sliceDir,
