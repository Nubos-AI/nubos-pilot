...
const { makeRepo } = require('./__tests__/fixtures.cjs');

test('recovery restores staged and unstaged changes to HEAD without touching other files', () => {
  const root = makeRepo();
  const run = (...args) => execFileSync('git', args, { cwd: root, encoding: 'utf-8' }).trim();
  writeFile(root, 'tracked.txt', 'original');
  writeFile(root, 'other.txt', 'original');
  run('add', '.');
  run('commit', '-qm', 'baseline');
  writeFile(root, 'tracked.txt', 'staged');
  writeFile(root, 'other.txt', 'user change');
  run('add', '.');
  writeFile(root, 'tracked.txt', 'unstaged');
  inRepo(root, () => git.restoreFiles(['tracked.txt']));
  assert.equal(fs.readFileSync(path.join(root, 'tracked.txt'), 'utf-8'), 'original');
  assert.equal(run('diff', '--cached', '--', 'tracked.txt'), '');
  assert.equal(run('diff', '--', 'tracked.txt'), '');
  assert.equal(fs.readFileSync(path.join(root, 'other.txt'), 'utf-8'), 'user change');
  assert.match(run('diff', '--cached', '--', 'other.txt'), /user change/);
});

test('task lookup and bulk undo exclude commits only reachable on other branches', () => {
  const root = makeRepo();
  const run = (...args) => execFileSync('git', args, { cwd: root, encoding: 'utf-8', stdio: 'pipe' }).trim();
  const id = 'M001-S001-T0001';
  run('checkout', '-b', 'other');
  run('commit', '--allow-empty', '-qm', `task(${id}): unrelated branch`);
  run('checkout', 'main');
  assert.throws(() => git.findCommitByTaskId(id, root), { code: 'task-commit-not-found' });
  assert.deepEqual(git.listTaskCommits('M001', root), []);
  run('merge', '--ff-only', 'other');
  assert.equal(git.findCommitByTaskId(id, root), run('rev-parse', 'HEAD'));
  assert.equal(git.listTaskCommits('M001', root).length, 1);
});
...
test('GIT-DIFF-3: gitDiffNoColor output strips ANSI even with color.ui=always (Pitfall 6)', () => {
  ...
  inRepo(root, () => {
    ...
    assert.ok(diff.includes('-red'));
    assert.ok(diff.includes('+green'));
