...
test('LAYOUT-5: path helpers return expected LAYOUT-v2 layout (no slug in slice/task dir)', () => {
  ...
  assert.equal(layout.milestoneResearchPath(1, root),
    path.join(np, 'milestones', 'M001', 'M001-RESEARCH.md'));
  assert.equal(layout.milestoneArchitecturePath(1, root),
    path.join(np, 'milestones', 'M001', 'M001-ARCHITECTURE.md'));
