...
// P3.3: an ff-merge that advances nothing exits 0 with "Already up to date".
// That is precisely what a commit landing on the calling branch instead of the
// worktree looks like — the isolation no-op passed through here silently and
// the worktree was then removed as if it had been merged.
test('WT-16: ffMergeSliceWorktree refuses a no-op merge that advances nothing (P3.3)', () => {
  const root = makeRepo();
  // Slice branch created but nothing committed inside the worktree — the shape
  // you get when commit-task ran without --cwd.
  worktree.createSliceWorktree('M001-S001', root);

  assert.throws(
    () => worktree.ffMergeSliceWorktree('M001-S001', 'main', root),
    (err) => err.name === 'NubosPilotError' && err.code === 'worktree-ff-no-op',
  );
});

test('WT-22: ffMergeSliceWorktree with allowEmpty accepts a no-op merge (gitignored-only slice)', () => {
  const root = makeRepo();
  worktree.createSliceWorktree('M001-S001', root);

  const res = worktree.ffMergeSliceWorktree('M001-S001', 'main', root, { allowEmpty: true });
