...
test('TA-30: isDispatchableTaskStatus excludes the terminal and parked statuses', () => {
  assert.equal(tasks.isDispatchableTaskStatus('pending'), true);
  assert.equal(tasks.isDispatchableTaskStatus('in-progress'), true);
  assert.equal(tasks.isDispatchableTaskStatus('done'), false);
  assert.equal(tasks.isDispatchableTaskStatus('skipped'), false);
  assert.equal(tasks.isDispatchableTaskStatus('parked'), false);
});

test('TA-31: every undispatchable status is a member of the status enum', () => {
  for (const s of tasks.TASK_UNDISPATCHABLE_STATUSES) {
    assert.ok(tasks.TASK_STATUS_ENUM.has(s), s + ' must be a valid task status');
  }
});
