...
const RUNTIMES = [
  ...
  },
  {
    ...
    payloadSubdir: 'nubos-pilot',
    skillsSubdir: 'skills',
    // Two surfaces, because Codex has two and they mean different things.
    //
    // Workflows are user-invoked procedures → skills. Codex loads custom prompts
    // only from $CODEX_HOME/prompts, which is user-global and deprecated;
    // `.codex/skills/` is project-scoped, so `commandsSubdir` stays unset and
    // lib/install/codex-skills.cjs renders them there.
    //
    // Agent roles are spawned, not typed → subagent TOML in `.codex/agents/`,
    // rendered by lib/install/codex-agents.cjs. That is what gives the loop its
    // isolation on Codex: a spawned subagent runs in its own thread, so a critic
    // does not share context with the executor it reviews.
    deliverAsSkills: { commands: true, prefix: 'np-' },
    agentsSubdir: 'agents',
