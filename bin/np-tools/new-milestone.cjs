...
function _interviewPayload(cwd) {
  ...
  return {
    ...
    questions: [
      ...
      { key: 'requirements', type: 'input',
        question: _requirementsQuestion(cwd) },
...
function _requirementsQuestion(cwd) {
  const root = path.resolve(cwd);
  let open = [];
  try {
    const { doc } = parseRoadmap(root);
    open = requirements.unassignedIds(root, doc);
  } catch { open = []; }
  const base = 'Which requirement IDs does this milestone cover? '
    + 'Comma-separated, empty to decide later — but note that an empty list makes '
    + '/np:validate-phase audit nothing.';
  if (!open.length) return base;
  return base + ' Unassigned right now: ' + open.slice(0, 20).join(', ')
...
function _validateAnswers(a) {
  ...
  if ('requirements' in a && a.requirements !== null
      && typeof a.requirements !== 'string' && !Array.isArray(a.requirements)) {
    throw new NubosPilotError(
      'answers-invalid-field',
      'requirements must be a comma-separated string or an array of requirement ids',
      { field: 'requirements', value: a.requirements },
    );
  }
}

function _resolveRequirements(root, answers) {
  const raw = answers.requirements;
  if (raw === undefined || raw === null || raw === '') return [];
  return requirements.assertIds(raw, root);
}
...
function _addMilestoneToRoadmap(root, mNum, answers) {
  ...
  return withFileLock(roadmapPath, () => {
    ...
    doc.milestones.push({
      ...
      status: 'pending',
      requirements: Array.isArray(answers._requirement_ids) ? answers._requirement_ids.slice() : [],
      success_criteria: [],
      slices: [],
    });
...
function _apply(answersPath, cwd, stdout) {
  ...
  answers._requirement_ids = _resolveRequirements(root, answers);
