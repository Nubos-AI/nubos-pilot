...
function _collectPhases(phases) {
  const out = [];
  if (!Array.isArray(phases)) return out;
  for (const ph of phases) {
    if (!ph || ph.number == null || ph.number === '') continue;
    if (ph.id === 'backlog' || ph.status === 'backlog') continue;
    ...
    out.push({
      ...
      status: ph.complete ? 'complete' : (ph.status || 'pending'),
...
async function _buildStats(cwd) {
  ...
  const phases = _collectPhases(roadmap ? roadmap.phases : []);
  let plansTotal = 0;
  let plansComplete = 0;
  for (const ph of phases) {
