...
function _setStatus(sandbox, mNum, sNum, tNum, status) {
  const mId = 'M' + String(mNum).padStart(3, '0');
  const sId = 'S' + String(sNum).padStart(3, '0');
  const tId = 'T' + String(tNum).padStart(4, '0');
  const p = path.join(sandbox, '.nubos-pilot', 'milestones', mId, 'slices', sId, 'tasks', tId, tId + '-PLAN.md');
  fs.writeFileSync(p, fs.readFileSync(p, 'utf-8').replace(/^status: .*$/m, 'status: ' + status));
}

test('EM-14: init excludes done/skipped/parked tasks from dispatchable_task_ids', () => {
  const sandbox = makeSandbox();
  seedRoadmapYaml(sandbox, _roadmap());
  seedMilestoneDir(sandbox, 1, {});
  seedSliceDir(sandbox, 1, 1, {});
  _seedTask(sandbox, 1, 1, 1, ['src/a.ts']);
  _seedTask(sandbox, 1, 1, 2, ['src/b.ts']);
  _seedTask(sandbox, 1, 1, 3, ['src/c.ts']);
  _seedTask(sandbox, 1, 1, 4, ['src/d.ts']);
  _setStatus(sandbox, 1, 1, 1, 'done');
  _setStatus(sandbox, 1, 1, 2, 'skipped');
  _setStatus(sandbox, 1, 1, 3, 'parked');

  const payload = subcmd.run(['init', '1'], { cwd: sandbox, stdout: _capture().stub });
  const wave = payload.waves[0];
  assert.deepEqual(wave.dispatchable_task_ids, ['M001-S001-T0004']);
  assert.equal(wave.complete, false);
  assert.equal(wave.task_count, 4);
  assert.equal(wave.tasks.length, 4);
});

test('EM-15: init marks a fully finished wave complete with an empty dispatch list', () => {
  const sandbox = makeSandbox();
  seedRoadmapYaml(sandbox, _roadmap());
  seedMilestoneDir(sandbox, 1, {});
  seedSliceDir(sandbox, 1, 1, {});
  seedSliceDir(sandbox, 1, 2, {});
  _seedTask(sandbox, 1, 1, 1, ['src/a.ts']);
  _seedTask(sandbox, 1, 2, 1, ['src/b.ts']);
  _setStatus(sandbox, 1, 1, 1, 'done');

  const payload = subcmd.run(['init', '1'], { cwd: sandbox, stdout: _capture().stub });
  assert.equal(payload.waves[0].complete, true);
  assert.deepEqual(payload.waves[0].dispatchable_task_ids, []);
  assert.equal(payload.waves[1].complete, false);
  assert.deepEqual(payload.waves[1].dispatchable_task_ids, ['M001-S002-T0001']);
});

test('EM-16: init leaves an empty slice not complete', () => {
  const sandbox = makeSandbox();
  seedRoadmapYaml(sandbox, _roadmap());
  seedMilestoneDir(sandbox, 1, {});
  seedSliceDir(sandbox, 1, 1, {});
  ...
  const payload = subcmd.run(['init', '1'], { cwd: sandbox, stdout: _capture().stub });
  assert.equal(payload.waves[0].task_count, 0);
  assert.equal(payload.waves[0].complete, false);
});

test('EM-17: execute-task emits the task status', () => {
  const sandbox = makeSandbox();
  seedRoadmapYaml(sandbox, _roadmap());
  seedMilestoneDir(sandbox, 1, {});
  seedSliceDir(sandbox, 1, 1, {});
  ...
  const payload = subcmd.run(['execute-task', '1', 'M001-S001-T0001'], { cwd: sandbox, stdout: _capture().stub });
  assert.equal(payload.status, 'pending');
});

for (const status of ['done', 'skipped', 'parked']) {
  test('EM-18/' + status + ': execute-task refuses a ' + status + ' task', () => {
    const sandbox = makeSandbox();
    seedRoadmapYaml(sandbox, _roadmap());
    seedMilestoneDir(sandbox, 1, {});
    seedSliceDir(sandbox, 1, 1, {});
    _seedTask(sandbox, 1, 1, 1, ['src/a.ts']);
    _setStatus(sandbox, 1, 1, 1, status);

    const cap = _capture();
    assert.throws(
      () => subcmd.run(['execute-task', '1', 'M001-S001-T0001'], { cwd: sandbox, stdout: cap.stub }),
      (err) => err
        && err.code === 'execute-milestone-task-not-dispatchable'
        && err.details.status === status,
    );
    assert.equal(cap.get(), '');
  });
}

test('EM-19: execute-task --allow-redo overrides the not-dispatchable guard', () => {
  const sandbox = makeSandbox();
  seedRoadmapYaml(sandbox, _roadmap());
  seedMilestoneDir(sandbox, 1, {});
  seedSliceDir(sandbox, 1, 1, {});
  ...
  const payload = subcmd.run(
    ['execute-task', '1', 'M001-S001-T0001', '--allow-redo'],
    { cwd: sandbox, stdout: _capture().stub },
  );
  assert.equal(payload.task_id, 'M001-S001-T0001');
  assert.equal(payload.status, 'done');
});

test('EM-20: execute-task still dispatches an in-progress task', () => {
  const sandbox = makeSandbox();
  seedRoadmapYaml(sandbox, _roadmap());
  seedMilestoneDir(sandbox, 1, {});
  seedSliceDir(sandbox, 1, 1, {});
  _seedTask(sandbox, 1, 1, 1, ['src/a.ts']);
  _setStatus(sandbox, 1, 1, 1, 'in-progress');
