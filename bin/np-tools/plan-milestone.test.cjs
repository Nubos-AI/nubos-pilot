...
test('PM-2b: init exposes the milestone-level RESEARCH/ARCHITECTURE paths (ADR-0032)', async () => {
  const sandbox = makeSandbox();
  seedRoadmapYaml(sandbox, _roadmap());
  const capAbsent = _capture();
  await subcmd.run(['init', '1'], { cwd: sandbox, stdout: capAbsent.stub });
  const absent = JSON.parse(capAbsent.get().trim());
  assert.ok(absent.milestone_research_path.endsWith(path.join('M001', 'M001-RESEARCH.md')));
  assert.ok(absent.milestone_architecture_path.endsWith(path.join('M001', 'M001-ARCHITECTURE.md')));
  assert.equal(absent.has_research, false);
  assert.equal(absent.has_architecture, false);

  seedMilestoneDir(sandbox, 1, {
    'M001-RESEARCH.md': '# research',
    'M001-ARCHITECTURE.md': '# adrs',
  });
  const capPresent = _capture();
  await subcmd.run(['init', '1'], { cwd: sandbox, stdout: capPresent.stub });
  const present = JSON.parse(capPresent.get().trim());
  assert.equal(present.has_research, true, 'plan-phase builds <files_to_read> from this flag');
  assert.equal(present.has_architecture, true);
});
