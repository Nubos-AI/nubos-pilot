...
function _appendRequirements(root, ids) {
  const p = path.join(root, '.nubos-pilot', 'REQUIREMENTS.md');
  const body = ids.map((id) => '- [ ] **' + id + '**: does a thing').join('\n') + '\n';
  fs.appendFileSync(p, '\n## Extra\n\n' + body);
}

function _roadmapMilestones(root) {
  const raw = fs.readFileSync(path.join(root, '.nubos-pilot', 'roadmap.yaml'), 'utf-8');
  return require('yaml').parse(raw).milestones;
}

test('NM-REQ-1 the interview asks for requirement ids', () => {
  const sandbox = makeEmptySandbox();
  const cap = _captureStdout();
  subcmd.run([], { cwd: sandbox, stdout: cap.stub });
  const payload = JSON.parse(cap.get().trim());
  const q = payload.questions.find((x) => x.key === 'requirements');
  assert.ok(q, 'interview must ask for requirement ids');
  assert.match(q.question, /validate-phase audit nothing/, 'the question must state the consequence of leaving it empty');
});

test('NM-REQ-2 the question offers the currently unassigned ids', () => {
  const sandbox = makeEmptySandbox();
  _seedInitializedProject(sandbox);
  _appendRequirements(sandbox, ['REQ-77', 'REQ-88']);
  const cap = _captureStdout();
  subcmd.run([], { cwd: sandbox, stdout: cap.stub });
  const q = JSON.parse(cap.get().trim()).questions.find((x) => x.key === 'requirements');
  assert.match(q.question, /REQ-77/);
  assert.match(q.question, /REQ-88/);
});

test('NM-REQ-3 supplied ids land in roadmap.yaml on the new milestone', () => {
  const sandbox = makeEmptySandbox();
  _seedInitializedProject(sandbox);
  _appendRequirements(sandbox, ['REQ-77', 'REQ-88']);
  const answers = Object.assign(_baseMsAnswers(), { requirements: 'REQ-77, REQ-88' });
  const cap = _captureStdout();
  subcmd.run(['--apply', _writeAnswers(sandbox, answers)], { cwd: sandbox, stdout: cap.stub });

  const emitted = JSON.parse(cap.get().trim());
  assert.deepEqual(emitted.requirements, ['REQ-77', 'REQ-88']);
  const created = _roadmapMilestones(sandbox).find((m) => m.id === emitted.milestone_id);
  assert.deepEqual(created.requirements, ['REQ-77', 'REQ-88']);
});

test('NM-REQ-4 an array of ids works as well as a comma string', () => {
  const sandbox = makeEmptySandbox();
  _seedInitializedProject(sandbox);
  _appendRequirements(sandbox, ['REQ-77']);
  const answers = Object.assign(_baseMsAnswers(), { requirements: ['req-77'] });
  const cap = _captureStdout();
  subcmd.run(['--apply', _writeAnswers(sandbox, answers)], { cwd: sandbox, stdout: cap.stub });
  assert.deepEqual(JSON.parse(cap.get().trim()).requirements, ['REQ-77']);
});

test('NM-REQ-5 an id absent from REQUIREMENTS.md is refused before anything is written', () => {
  const sandbox = makeEmptySandbox();
  _seedInitializedProject(sandbox);
  const before = _roadmapMilestones(sandbox).length;
  const answers = Object.assign(_baseMsAnswers(), { requirements: 'GHOST-99' });
  assert.throws(
    () => subcmd.run(['--apply', _writeAnswers(sandbox, answers)], { cwd: sandbox, stdout: _captureStdout().stub }),
    /no such requirement/,
  );
  assert.equal(_roadmapMilestones(sandbox).length, before, 'no milestone may be created on a refused id');
});

test('NM-REQ-6 the TBD placeholder is refused with a fix hint', () => {
  const sandbox = makeEmptySandbox();
  _seedInitializedProject(sandbox);
  _appendRequirements(sandbox, ['REQ-TBD']);
  const answers = Object.assign(_baseMsAnswers(), { requirements: 'REQ-TBD' });
  assert.throws(
    () => subcmd.run(['--apply', _writeAnswers(sandbox, answers)], { cwd: sandbox, stdout: _captureStdout().stub }),
    /replace it in REQUIREMENTS\.md/,
  );
});

test('NM-REQ-7 omitting requirements stays allowed and yields an empty list', () => {
  const sandbox = makeEmptySandbox();
  _seedInitializedProject(sandbox);
  const cap = _captureStdout();
  subcmd.run(['--apply', _writeAnswers(sandbox, _baseMsAnswers())], { cwd: sandbox, stdout: cap.stub });
  assert.deepEqual(JSON.parse(cap.get().trim()).requirements, []);
});

test('NM-REQ-8 a non-string non-array requirements value is refused', () => {
  const sandbox = makeEmptySandbox();
  _seedInitializedProject(sandbox);
  const answers = Object.assign(_baseMsAnswers(), { requirements: 42 });
  assert.throws(
    () => subcmd.run(['--apply', _writeAnswers(sandbox, answers)], { cwd: sandbox, stdout: _captureStdout().stub }),
