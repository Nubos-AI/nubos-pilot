...
const { markMilestoneStarted } = require('../../lib/rollup.cjs');
const { extractFrontmatter } = require('../../lib/frontmatter.cjs');
const { getAgentSkills } = require('../../lib/agents.cjs');
const { isDispatchableTaskStatus } = require('../../lib/tasks.cjs');
...
function _initPayload(mNum, cwd, opts) {
  ...
  for (const s of slices) {
    ...
    const dispatchable = tasks.filter((t) => isDispatchableTaskStatus(t.status));
    waves.push({
      wave: s.number,
      slice_id: s.id,
      ...
      dispatchable_task_ids: dispatchable.map((t) => t.id),
      complete: tasks.length > 0 && dispatchable.length === 0,
      tasks,
    });
  }
...
function run(args, ctx) {
  ...
  switch (verb) {
    case 'init': {
      ...
      let started = null;
      try {
        started = markMilestoneStarted(mNum, cwd);
      } catch (err) {
        try {
          require('../../lib/logger.cjs').child('execute-milestone').error(
            'could not mark milestone in-progress', {
              event: 'milestone-start-status-failed',
      ...
      payload.milestone_status_started = started;
    ...
    case 'execute-task': {
      ...
      if (!isDispatchableTaskStatus(payload.status) && !list.includes('--allow-redo')) {
        throw new NubosPilotError(
          'execute-milestone-task-not-dispatchable',
          'Task ' + taskId + ' has status "' + payload.status + '" and must not be re-executed. '
            + 'Reset it first (`/np:undo-task ' + taskId + '` for a committed task, `/np:unpark` for a parked one), '
            + 'or pass --allow-redo for an explicit override.',
