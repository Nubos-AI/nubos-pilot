...
const V2_STATE_MD = ['---', 'milestone: M001', '---', '', '# State', ''].join('\n');

test('ST-V2-1: a v2 roadmap yields phases at all', async () => {
  const root = makeSandbox(V2_VERIFIED_YAML, V2_STATE_MD);
  const sink = makeSink();
  await statsCli.run(['json'], { cwd: root, stdout: sink });
  const payload = JSON.parse(sink.toString());
  assert.equal(payload.phases.length, 3, 'v2 milestones must be collected, not silently dropped');
});

test('ST-V2-2: verified counts as complete and non-terminal statuses survive verbatim', async () => {
  const root = makeSandbox(V2_VERIFIED_YAML, V2_STATE_MD);
  const sink = makeSink();
  await statsCli.run(['json'], { cwd: root, stdout: sink });
  const byNumber = {};
  for (const ph of JSON.parse(sink.toString()).phases) byNumber[ph.number] = ph.status;

  assert.equal(byNumber['1'], 'complete', 'verified is a terminal status, not pending');
  assert.equal(byNumber['2'], 'failed', 'failed must not be degraded to pending');
  assert.equal(byNumber['3'], 'pending');
});
