...
function seedTask(root, taskId, files) {
  return seedTaskFixture(root, taskId, { status: 'in-progress', files });
}
...
test('RS-6: restoreFiles failure must not be reported as restored_files (payload truth)', () => {
  ...
  const checkpoint = require('../../lib/checkpoint.cjs').checkpointPath('M006-S001-T0001', root);
  fs.mkdirSync(path.dirname(checkpoint), { recursive: true });
  fs.writeFileSync(checkpoint, JSON.stringify({ task_id: 'M006-S001-T0001' }));
  ...
  try {
    ...
    assert.ok(fs.existsSync(checkpoint), 'failed restore must retain its checkpoint');
    assert.deepEqual(payload.deleted_checkpoints, []);
    assert.equal(require('../../lib/state.cjs').readState(root).frontmatter.current_task, 'M006-S001-T0001');
  } finally {
    process.chdir(prev);
  }
});
