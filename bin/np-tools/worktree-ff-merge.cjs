...
function _parseArgs(args) {
  const out = { sliceFullId: null, target: null, allowEmpty: false };
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a === '--target') { out.target = args[++i] || null; continue; }
    if (a === '--allow-empty') { out.allowEmpty = true; continue; }
...
function run(args, opts) {
  ...
  const result = ffMergeSliceWorktree(parsed.sliceFullId, parsed.target, cwd, { allowEmpty: parsed.allowEmpty });
