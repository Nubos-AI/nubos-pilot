...
function run(args, ctx) {
  ...
  if (files.length > 0) {
    try {
      ...
      // Keep recovery state and the worktree available for a retry.
      const payload = {
        ok: false, task_id: taskId, restored_files: [],
        deleted_checkpoints: [], worktree_removed: null, errors,
        message: 'reset-slice incomplete — restore failed; checkpoint and worktree retained',
      };
      stdout.write(JSON.stringify(payload) + '\n');
      return payload;
    }
