...
async function _initPayload(mNum, cwd, opts) {
  ...
  const mResearchPath = layout.milestoneResearchPath(mNum, cwd);
  const mArchitecturePath = layout.milestoneArchitecturePath(mNum, cwd);
  ...
  return {
    ...
    milestone_research_path: mResearchPath,
    milestone_architecture_path: mArchitecturePath,
    name: def.name || '',
    goal: def.goal || '',
    ...
    has_research: fs.existsSync(mResearchPath),
    has_architecture: fs.existsSync(mArchitecturePath),
