...
npm run test:coverage     # full suite and 70% line-coverage gate in one run
npm run ci                # same single run, used by CI
npm run check-coverage    # compatibility alias for the coverage run
```

All full-suite commands use the same file patterns in `bin/run-tests.cjs`,
including the root dispatcher tests. Node enforces the coverage threshold and
streams test failures directly; CI does not rerun the library tests separately.
Use `lib/__tests__/fixtures.cjs` for shared temporary repositories, task plans,
and output capture. Keep distinct regression cases even when their setup is shared.
